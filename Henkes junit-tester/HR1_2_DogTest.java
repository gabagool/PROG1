...
/**
 * Testfall för hundklassen i uppgift HR1.2.
 * <p>
 ...
 * täckt. Så, varför finns det överhuvudtaget?
 * <p>
 * Svaret ligger i hur testen är framtagna. De tidigare testen är medvetet
 * designade för att för att försöka hitta ett visst potentiellt fel. Testdatat
 * i detta test är istället skapat med en slumpgenerator. Testfall skapade med
 * hjälp av slump har många nackdelar jämfört med manuellt designade, inte minst
 * att det kan vara svårt att veta vad som testats och att köra om samma test
 * igen. En nackdel med manuellt designade test är dock att det är lätt att
 * fastna i ett visst sätt att tänka, och på så sätt missa att testa vissa
 * saker. Test baserade på slump kan, om man har tur, undvika detta.
 * <p>
 * Liknande test finns i många av de kommande uppgifterna. De är alltid de sista
 * av de funktionella testen, och testar ofta lite större scenarier.
 * 
 * @author Henrik Bergström
 * @version 2023-11-03 12:21
 * @see HR1_1_OwnerTest
 */
...
public class HR1_2_DogTest {
	...
	public static final String VERSION = "2023-11-03 12:21";
	...
	public void ableToIncreaseAge() {
		...
		assertEquals(dog.getAge(), DEFAULT_AGE + 1);

		dog = new Dog(DEFAULT_NAME, DEFAULT_BREED, DEFAULT_AGE, DEFAULT_WEIGHT);
		callAssumedMethodForIncreasingAgeOfDog(dog, 3);
		assertEquals(dog.getAge(), DEFAULT_AGE + 3);
	}
	...
	public void unableToDecreaseAge() {
		...
		dog = new Dog(DEFAULT_NAME, DEFAULT_BREED, DEFAULT_AGE, DEFAULT_WEIGHT);
		callAssumedMethodForIncreasingAgeOfDog(dog, -1);
		assertEquals(dog.getAge(), DEFAULT_AGE);
	}
	...
	public void unableToDecreaseAgeThroughOverflow() {
		...
		callAssumedMethodForIncreasingAgeOfDog(dog, 1);
		assertEquals(dog.getAge(), Integer.MAX_VALUE);
	}
	...
	public void weightSetByCtr() {
		...
		assertEquals(dog.getWeight(), 1);

		dog = new Dog(DEFAULT_NAME, DEFAULT_BREED, DEFAULT_AGE, 10);
		assertEquals(dog.getWeight(), 10);
	}
	...
	public void taillengthCorrectForNonDachshunds() {
		...
		assertEquals(dog.getTailLength(), 2.0, 0.01, "(±0.01)");

		dog = new Dog(DEFAULT_NAME, DEFAULT_BREED, DEFAULT_AGE, DEFAULT_WEIGHT);
		assertEquals(dog.getTailLength(), DEFAULT_TAIL_LENGTH, 0.01, "(±0.01)");
	...
	public void taillengthCorrectForSwedishDachshunds() {
		...
		dog = new Dog(DEFAULT_NAME, "Tax", DEFAULT_AGE, DEFAULT_WEIGHT);
		assertEquals(dog.getTailLength(), 3.7, 0.01, "(±0.01)");
	...
	public void taillengthCorrectForNonDachshundsAfterIncreasingAge() {
		...
		assertEquals(dog.getTailLength(), 11.0, 0.01, "(±0.01)");
	}
